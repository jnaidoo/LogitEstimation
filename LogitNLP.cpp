// LogitNLP.cpp
// implementation of LogitNLP interface
//
/** VERSION HISTORY:
  * ----------------
...
#include <cassert>
// <iostream> already included in Logit.hpp
...
bool LogitNLP::get_nlp_info(Index& n, Index& m, Index& nnz_jac_g,
...
bool LogitNLP::get_bounds_info(Index n, Number* x_l, Number* x_u,
...
bool LogitNLP::get_starting_point(Index n, bool init_x, Number* x,
...
bool LogitNLP::eval_f(Index n, const Number* x, bool new_x, Number& obj_value)
{
  assert(n == 4);
  // to be altered later: is Number* a C-style array?
  // if so, can we simply use Eigen::Map calcObjective?
   coefficientVector4d_t tmpBeta;
  tmpBeta << x[0], x[1], x[2], x[3];
  obj_value = myLogit.calcObjective(tmpBeta); 
  return true;
}

// return the gradient of the objective function grad_{x} f(x)
bool LogitNLP::eval_grad_f(Index n, const Number* x, bool new_x, Number* grad_f)
{
  assert(n == 4);
   coefficientVector4d_t tmpBeta;
   tmpBeta << x[0], x[1], x[2], x[3];
...
bool LogitNLP::eval_g(Index n, const Number* x, bool new_x, Index m, Number* g)
{
...
bool LogitNLP::eval_jac_g(Index n, const Number* x, bool new_x,
...
bool LogitNLP::eval_h(Index n, const Number* x, bool new_x,
...
void LogitNLP::finalize_solution(SolverReturn status,
